<template>
  <div class="releasetime">
    <div class="top">
      ...
      <p>
        <i class="el-icon-warning"></i>请您务必提前24小时发布黄色被预占时间，以免学生不能正常上课
      </p>
    ...
    <table border="1" cellspacing="0">
      <tr v-for="(time,timeidx) in timeslot" :key="time">
        <td v-for="(date,dateidx) in tableweek" :key="date">
...
export default {
  ...
  methods: {
    tabtime(idx) {
      //切换上周下周回到本周
      this.tabtype.forEach((item, index) => {
        //改变高亮
        if (idx === index) {
          item.flag = true;
        } else {
    ...
    lastclick() {
      this.weekdate = [""];
      this.weekdate = this.setDateFn(
        this.addDate(this.currentFirstDate, this.weeknum)
      );
    },
